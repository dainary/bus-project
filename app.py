...
df = pd.read_excel("불편도 코딩 엑셀자료.xlsx")
...
# 매칭 실패 확인
failed = result[result["위도"].isna()]
print(f"매칭 실패 개수: {len(failed)}")

if len(failed) > 0:
    print("\n매칭 실패 정류장:")
    print(failed["bus stop"].tolist())

# 저장
result.to_excel(
    "dalseo_bus_stop_with_location.xlsx",
    index=False
)
